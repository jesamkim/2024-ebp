...
def get_driver():
    ...
    options.binary_location = '/opt/chrome/chrome' # 크롬 브라우저 경로 지정
...
def lambda_handler(event, context):
    ...
    df.to_csv(f"/tmp/{today}_origin.csv", encoding="utf-8-sig")  # 일별 파일명설정
    df.to_excel(f"/tmp/{today}_origin.xlsx")
    ...
    df0.to_csv(f"/tmp/{today}_news_result.csv", encoding="utf-8-sig")

    df0.to_excel(f"/tmp/{today}_news_result.xlsx")
    ...
    for filename in os.listdir(f'/tmp/'):
        if filename.endswith('.xlsx'):
            local_path = os.path.join(f'/tmp/', filename)
